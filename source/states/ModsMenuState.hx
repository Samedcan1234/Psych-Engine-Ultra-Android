...
{
	// ═══════════════════════════════════════════════════════
	// 🎮 ORİJİNAL SİSTEM DEĞİŞKENLERİ (Korundu)
	// ═══════════════════════════════════════════════════════
	var bg:FlxSprite;
	var icon:FlxSprite;
	...
	override function create()
	{
		...
		#if DISCORD_ALLOWED
		DiscordClient.changePresence("Mod Merkezi - XQ Edition", null);
		#end
		...
		if (modsList.all.length < 1)
		{
			...
			noModsTxt = new FlxText(0, 0, FlxG.width - RIGHT_W, Language.getPhrase('no_mods_installed', 'MOD YÜKLENMEDİ\nÇIKMAK İÇİN GERİ TUŞUNA BASIN'), 32);
	...
	function createShowcasePanel()
	{
		...
		// Sağ kenar çizgisi (sol-sağ panel ayırıcı)
	...
	function createControlHints()
	{
		...
		controlHintsText = new FlxText(0, FlxG.height - 22, FlxG.width - RIGHT_W, "↑↓: Seç  |  ►: Butonlar  |  ESC: Geri  |  HOME/END: İlk/Son", 11);
	...
	override function update(elapsed:Float)
	{
		...
		// ── Orijinal Input sistemi (aynen korundu) ─────────────
		...
		if (modsList.all.length > 0)
		{
			...
			if (lastMode == hoveringOnMods)
			{
				if (hoveringOnMods)
				{
					if (controls.UI_RIGHT_P) { hoveringOnMods = false; var button = getButton(); button.ignoreCheck = button.onFocus = false; curSelectedButton = 0; changeSelectedButton(); }
				}
				else
				{
					...
					{
						if (controls.UI_UP_P) { switch(curSelectedButton) { case -2: curSelectedMod = 0; hoveringOnMods = true; var button = getButton(); button.ignoreCheck = button.onFocus = false; changeSelectedMod(); case -1: changeSelectedButton(-1); } }
						else if (controls.UI_DOWN_P) { switch(curSelectedButton) { case -2: changeSelectedButton(1); case -1: curSelectedMod = 0; hoveringOnMods = true; var button = getButton(); button.ignoreCheck = button.onFocus = false; changeSelectedMod(); } }
	...
	// ═══════════════════════════════════════════════════════
	// 📜 LİSTE SCROLL (Manuel — kamera kaymadan)
	// ═══════════════════════════════════════════════════════
	function updateListScroll(elapsed:Float)
	{
		...
		var visibleEnd:Float   = FlxG.height - DOCK_H - 4;
		...
		// Öğeleri offset'e göre konumlandır
		updateItemPositions();

		// Seçim çubuklarını güncelle
		var rx:Float = FlxG.width - RIGHT_W;
	...
	// ═══════════════════════════════════════════════════════
	// 🔄 ITEM POZİSYONLARI (Orijinal mantık + scroll offset)
	// ═══════════════════════════════════════════════════════
	...
	// ═══════════════════════════════════════════════════════
	// ORİJİNAL FONKSİYONLAR (Korundu + modernize)
	// ═══════════════════════════════════════════════════════
